# Generated by Anaconda 42.27.12
# Generated by pykickstart v3.62

# See: https://fedora.gitlab.io/docs/fedora-linux-documentation/install-guide/fedora/rawhide/install-guide/appendixes/Kickstart_Syntax_Reference.html#sect-kickstart-commands-ostreesetup
#version=DEVEL

%post --erroronfail
cp /etc/skel/.bash* /root
%end

# Install Interface
graphical

# Keyboard layouts
keyboard --vckeymap=us --xlayouts='us'
# System language
lang en_US.UTF-8

# Run the Setup Agent on first boot
firstboot --enable

# Generated using Blivet version 3.12.1
ignoredisk --only-use=mmcblk0
autopart
# Partition clearing information
clearpart --all --initlabel

# System timezone
timezone America/New_York --utc

#Root password
rootpw --lock
user --groups=wheel --name=user --password=$y$j9T$sXtede5NuJBxYAZ/OEdZAo2Z$.g7xjN8uGx9uWZp.ioc70crrYrqAcq13/EQX6W6H6x6 --iscrypted --gecos="User"

# # Package Installations
# %packages --nobase
#     # Install all core packages
#     @core

#     # Browsers
#     # firefox
#     google-chrome-stable

#     # Office Suite
#     libreoffice
# %end